...
params:
  ...
  max_targets:
    ...
    max: 4861
    value: 120
    label: "Maxmimum number of unique Targets"
  min_strength:
    input: numeric
    value: 0.2
    min: 0
  ...
  WSconditions:
    ...
    value: 5
    min: 1
    label: "Number of Within-subject conditions"
---
<style>
...
#### Banned Words
List of free responses that are banned from use in this experiment (potentially offensive).
```{r banned_words}
banned <- c("devil", "slave")
```
...
1. Join the norms and word frequency data together, keeping only cue-response pairs where the response words meet the length and word frequency constraints.
2. Only include responses which have at least `r params$n_cue` cue(s) with forward associations of at least `r params$min_strength`
...
filtered_norms <- inner_join(norms, WF, by = c("response"="word")) %>%
  filter(!cue %in% c("president", banned),
         !response %in% banned,
...
Select the top `r params$n_cue` cues to each associate to finalize the stimuli set.
```{r top_n_cues}
filtered_norms <- filtered_norms %>%
  group_by(response) %>%
...
```{r similarity}
remove_responses <- c("angry", "build", "marry", "scared", "scary","cloth")
filtered_norms <- filtered_norms[!filtered_norms$response %in% remove_responses, ]

filtered_norms <- summarise(group_by(filtered_norms, response),
                            mean_FAS = mean(forward_association)) %>%
  top_n(params$max_targets, mean_FAS) %>%
  slice(1:params$max_targets) %>% # to break ties from top_n
  semi_join(filtered_norms, y=., "response")

n_unique_targets <- length(unique(filtered_norms$response))
n_usable_targets <- n_unique_targets - (n_unique_targets %% params$WSconditions)
```
...
3. Each cue has a minimum forward association of `r params$min_strength`.
...
7. There are at least `r params$n_cue` cues for each response
...
Assuming that there are `r params$WSconditions` experimental conditions, and stimuli are divided evenly between these conditions, then the maximum number of trials per condition is <b> `r n_usable_targets/params$WSconditions` </b> .
